...
overall_rating_of_instruction <- dat %>% group_by(school, subject, instructor) %>%
...
amount_stimulated_by_instructor <- dat %>% group_by(school, subject, instructor) %>%
...
View(head(amount_stimulated_by_instructor %>% arrange(avg_stimulated), n = 30))
View(head(amount_stimulated_by_instructor %>% arrange(desc(avg_stimulated)), n = 30))
...
# EECS
View(head(overall_rating_of_instruction %>% arrange(avg_instruction) %>% filter(subject == "EECS"), n = 30))
View(head(overall_rating_of_instruction %>% arrange(desc(avg_instruction)) %>% filter(subject == "EECS"), n = 30))

# SESP
View(head(amount_challenged_by_course %>% filter(school == "SESP" & 
